...
class DEMC(Sampler):
    def __init__(self, models, theta0s, dataloaders, sigmas, num_chains=10, c=0.1, schedule=lambda n, num_iterations: 1.,
    storage='list', keys=['theta', 'target_val', 'accepted'], path=os.getcwd(), mode='a'):
        ...
        for i in range(self.num_chains):
            self.samplers.append(MetropolisHastings(
                self.models[i], theta0s[i], self.dataloaders[i], symmetric=True,
                kernel=DEMCKernel(self.sigmas[i], c=0.1, dtype=self.models[i].dtype, device=self.models[i].device),
                chain=self.chains[i]
            ))
