...
class SMMALA(SerialSampler):
    ...
    def __init__(self, model, theta0, dataloader, step=0.1, transform=None, keys=['theta', 'target_val', 'accepted'], dtype=torch.float64):
    ...
    def reset(self, theta):
        ...
        self.current['target_val'], self.current['grad_val'], self.current['metric_val'] = self.model.upto_metric_log_target(self.current['theta'].clone().detach(), data, label)
    ...
    def draw(self, savestate=False):
        ...
        for data, label in self.dataloader:
            ...
            proposed['theta'] = proposal_mean + np.sqrt(self.step) * self.current['chol_inv_metric_val'] @ torch.randn(num_params, dtype=dtype)
            proposed['target_val'], proposed['grad_val'], proposed['metric_val'] = self.model.upto_metric_log_target(proposed['theta'].clone().detach(), data, label)
            if self.transform is not None:
            ...
            log_rate = log_rate + 0.5 * (inv_metric_logdet + (loc_minus_proposal_mean.t() @ (self.current['metric_val'] @ loc_minus_proposal_mean))/self.step)
            ...
            log_rate = log_rate - 0.5 * (inv_metric_logdet +(loc_minus_proposal_mean.t() @ (proposed['metric_val'] @ loc_minus_proposal_mean))/self.step)

            if torch.log(torch.rand(1, dtype=dtype)) < log_rate:
            ...
            if verbose:
                print("self.current['accepted']:", self.current['accepted'])
