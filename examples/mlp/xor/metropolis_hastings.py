...
kernel = NormalKernel(
    torch.zeros(num_params, dtype=torch.float64),
    proposal_scale * torch.ones(num_params, dtype=torch.float64)
)
sampler = MetropolisHastings(model, theta0, dataloader, kernel=kernel)
